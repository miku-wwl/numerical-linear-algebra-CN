%PMINRES方法的程序-pminres2.m
function [x,iter,time,res,resvec]=pminres2(A,b,x,M1,M2,max_it,tol)
%预处理极小残量法求解对称不定方程组Ax=b
tic; r=M2\(M1\(b-A*x)); b=norm(r); v=r/b; bt=b;
z=M2\(M1\(A*v)); a=v'*z; u=z-a*v; b1=norm(u); 
if (b1~=0),  v1=u/b1; end
...
while (iter<max_it)
    ...
    z=M2\(M1\(A*v1)); a=v1'*z; u=z-a*v1-b1*v; b2=norm(u);
    if (b2~=0),  v2=u/b2;   end
