%GMRES法
function [x, k, time, res, resvec, flag] = cpgmres( A, b, x, M, max_it, tol )
...
for k = 1 : max_it,   
    ...
    [c(k),s(k),H(k,k)] = cgivens( H(k,k), H(k+1,k) ); % form j-th Givens rotation matrix
    % approximate residual norm
    xi(k+1) = -conj(s(k))*xi(k);
    xi(k)   = c(k)*xi(k);  
